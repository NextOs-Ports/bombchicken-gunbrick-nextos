#!/usr/bin/env bash
# Deterministic public AArch64 build for Bomb Chicken.
...
OUTPUT=${BC_UNIVERSAL_OUTPUT:-build/bombchicken-nextos}
...
fail() {
  printf 'bombchicken build error: %s\n' "$*" >&2
  exit 1
}

case $OUTPUT in
  /*|*../*|../*|*/..|..)
    fail "BC_UNIVERSAL_OUTPUT must stay inside the port directory"
    ;;
esac

if [[ ${BC_BUSTER_IN_CONTAINER:-0} != 1 ]]; then
  ...
  if [[ -z $NEXTOS_SYSROOT ]]; then
    ...
    NEXTOS_SYSROOT=$TOOLCHAIN/aarch64-libreelec-linux-gnu/sysroot
  fi
  ...
  exec docker run --rm --network none \
    -e BC_BUSTER_IN_CONTAINER=1 \
    -e BC_UNIVERSAL_OUTPUT="$OUTPUT" \
    -e BC_HOST_UID="$(id -u)" \
    -e BC_HOST_GID="$(id -g)" \
...
if [[ -n ${BC_HOST_UID:-} && -n ${BC_HOST_GID:-} ]]; then
  chown "$BC_HOST_UID:$BC_HOST_GID" "$OUTPUT" 2>/dev/null || true
fi

printf 'BOMBCHICKEN UNIVERSAL BUILD OK: %s\n' "$OUTPUT"
